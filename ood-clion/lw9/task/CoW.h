...
class CoW
{
    struct WriteProxy
    {
        explicit WriteProxy(Value* value) noexcept: value_ptr_{value}
        {}
...
public:
    ...
    CoW() : value_(std::make_shared<Value>())
    {}

    // Создаём значение за счёт перемещения его из value.
    explicit CoW(Value&& value) : value_(std::make_shared<Value>(std::move(value)))
    {}

    // Создаём значение из value.
    explicit CoW(const Value& value) : value_(std::make_shared<Value>(value))
    {}
